...
export default function FileUpload({ onFileSelect }) {
  ...
  const validate = (file) => {
    if (!ACCEPTED_TYPES.includes(file.type))
      return "Only JPEG, PNG, WEBP, TIFF and PDF files are accepted.";
      ...
      >
        ...
        {selectedFile ? (
          <div className="text-center">
            <p className="text-violet-300 font-semibold text-sm break-all">
              {selectedFile.name}
            </p>
          ...
          <div className="text-center">
            ...
            <p className="text-white/30 text-xs mt-3">
              JPEG · PNG · WEBP · TIFF · PDF &nbsp;—&nbsp; max {MAX_MB} MB
            </p>
          </div>
        )}
      </div>
